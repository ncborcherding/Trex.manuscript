...
for (i in seq_along(list)) {
  score <- list[[i]]
  cells <- score$unique.membership....barcode...
...
saveRDS(return, file = "aa.edge.rds")
```
...
ggsave("DimExamination.png", height = 6, width = 12, dpi = 600, bg = "transparent")

DefaultAssay(list) <- 'ADT.integrated'
VariableFeatures(list) <- rownames(list[["ADT.integrated"]])
list <- NormalizeData(list, normalization.method = 'CLR', margin = 2) %>% 
...
list <- RunUMAP(list, reduction = 'Trex.AF', dims = 3:30, 
              reduction.name = 'Trex.umap', reduction.key = 'trexUMAP_')
library(RColorBrewer)
pal <- colorRampPalette(brewer.pal(8, "Set1"))(length(unique(list$Patient)))
